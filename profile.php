...
if (isset($_SESSION["id"])) {
 ...
 <h3>Receptek:</h3>
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<div class='doboz '>
                        ...
                        <a href='recept.php?id=" . $row["id"] . "'>
                            <div class='col-sm-2'>
                                <img class='img-fluid' src='include/img/" . $row["id"] . ".jpg' style='border-radius: 30px 30px 30px 30px;'>
                            </div>
                            <div class='col-sm-10'>
                                <h1>" . $row["neve"] . "</h1>
                                <h5>" . $row["mikor"] . "</h5>
    ...
    if (!isset($_GET['id']) || $x > 0) {
        $sql4 = "SELECT ki,mit,recept.neve as hol, hozzaszolasok.id  from hozzaszolasok,recept where recept.id=hozzaszolasok.recept_id and hozzaszolasok.moderated=1 and ki=$id ORDER by id desc ";
        $result = $conn->query($sql4);
        ...
        <div class='mssgBox' >
         <h3>" . $row["hol"] . "</h3>
          ...
          ";
                if ($x == 1) {
                    $ki = $row["ki"];
                    $hozzaszolas_id = $row["id"];
                    echo "<form method='post' action='profile.php?id=$ki'> <button name=\"cloes_ticket\" type=\"submit\" value='$hozzaszolas_id' class=\"btn btn-primary\"> vétózás</button>
